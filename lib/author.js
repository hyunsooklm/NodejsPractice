...
exports.create = function (request, response) {
  ...
  if (request.method == "POST") {
    ...
    request.on('end', function () {
      post = qs.parse(body);
      db.query(`INSERT INTO AUTHOR(name,profile)VALUE(?,?);`, [post.name, post.profile], function (err, result) {
        if (err) {
          throw err;
        }
        else {
...
exports.update_process = function (request, response) {
  ...
  if (request.method == "POST") {
    ...
    request.on('end', function () {
      post = qs.parse(body);
      console.log(post);
      db.query('UPDATE AUTHOR SET NAME=?,PROFILE=? WHERE ID=?;', [post.name, post.profile, post.author_id], function (err, data) {
        if (err) {
          throw err;
        }
...
exports.delete_process = function (request, response) {
  ...
  if (request.method == "POST") {
    ...
    request.on('end', function () {
      post = qs.parse(body);
      console.log(post);
      db.query('DELETE FROM AUTHOR WHERE ID=?', [post.author_id], function (err1, result) {
        if (err1) {
          throw err1;
        }
        else {
          db.query('DELETE FROM TOPIC WHERE TOPIC.AUTHOR_ID=?', [post.author_id], function (err2, result2) {
            if (err2) {
              throw err2;
            }
            else {
